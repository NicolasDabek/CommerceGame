...
function openBidModal(offerId) {
  ...
  Modal.open({
    ...
    bodyHTML: `<p style="margin-bottom:12px">${currentInfo}</p><p class="text-muted" style="margin-bottom:16px;font-size:0.9rem">Quantité : ${offer.quantity} — L'argent sera bloqué jusqu'à la fin de l'enchère.</p><div class="form-group"><label>Votre enchère unitaire (€) — minimum ${formatMoney(minBid)} €</label><input type="number" id="bid-amount" class="input" step="0.01" min="${minBid}" value="${minBid.toFixed(2)}" style="width:100%" /></div><p class="text-muted" style="font-size:0.85rem;margin-top:8px">Total bloqué : <span id="bid-total">${formatMoney(minBid * offer.quantity)}</span> €</p>`,
    buttons: [
      { label: 'Annuler', className: 'btn-ghost', onClick: () => Modal.close() },
...
function initUI() {
  ...
  jobsUI = new JobsUI({
    ...
    onPolish: (itemId, quality, perfection) => {
      ...
      if (result.success) setStatus(`Réparé : ${result.name} Q${result.quality}${result.usedCopper ? ' +cuivre' : ''}`);
